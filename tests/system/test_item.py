...
class ItemTest(BaseTest):
    ...
    def test_get_item(self):
        ...
        with self.app() as client:
            with self.app_context():
                ...
                self.assertDictEqual({'name': 'test', 'price': 9.99}, json.loads(response.data))
    ...
    def test_create_item(self):
        ...
        with self.app() as client:
            with self.app_context():
                StoreModel('test').save_to_db()
                response = client.post('item/test', data={'price': 15.99, 'store_id': 1})
                self.assertEqual(response.status_code, 201)
                self.assertDictEqual({'name': 'test', 'price': 15.99}, json.loads(response.data))

    def test_create_duplicate_item(self):
        with self.app() as client:
            with self.app_context():
                StoreModel('test').save_to_db()
                ItemModel(name='test', price=15.99, store_id=1).save_to_db()
                response = client.post('item/test', data={'price': 15.99, 'store_id': 1})
                self.assertEqual(response.status_code, 400)
                self.assertDictEqual({'message': "An item with name '{}' already exists.".format('test')}, json.loads(response.data))
